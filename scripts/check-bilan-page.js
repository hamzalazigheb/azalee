...
async function checkPage() {
  try {
    ...
    const page = await PageContent.findOne({ path: 'patrimoine/bilan' });
    
    if (!page) {
      console.log('❌ Page "patrimoine/bilan" non trouvée dans MongoDB');
      ...
      if (page.content) {
        ...
        console.log(`   - pourquoi: ${page.content.pourquoi ? '✅' : '❌'}`);
        console.log(`   - contenu: ${page.content.contenu ? '✅' : '❌'}`);
        console.log(`   - tarifs: ${page.content.tarifs ? '✅' : '❌'}`);
